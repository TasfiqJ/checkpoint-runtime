...
export default function DemoPage() {
  ...
  const handleStart = async () => {
    ...
    try {
      // Find or wait for the auto-created run from workers
      addEvent('Waiting for training workers to create run...', 'info');

      // Poll for runs until one appears
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Watch fault-tolerant checkpointing in action. Kill a worker and see recovery.
          </p>
        </div>
        {!runId && (
          <button
        ...
        <>
          ...
          <div className="grid grid-cols-4 gap-4">
            <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
              <p className="text-xs text-gray-500 uppercase tracking-wider">Training Step</p>
              <p className="text-2xl font-mono font-bold text-gray-100 mt-1">{run.current_step}</p>
            </div>
            <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
              <p className="text-xs text-gray-500 uppercase tracking-wider">Checkpoints</p>
              <p className="text-2xl font-mono font-bold text-gray-100 mt-1">{committedCheckpoints.length}</p>
            </div>
            <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
              <p className="text-xs text-gray-500 uppercase tracking-wider">Data Saved</p>
              <p className="text-2xl font-mono font-bold text-gray-100 mt-1">{formatBytes(totalBytes)}</p>
            </div>
            <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
              <p className="text-xs text-gray-500 uppercase tracking-wider">Active Workers</p>
              <p className="text-2xl font-mono font-bold text-gray-100 mt-1">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            <h3 className="text-lg font-semibold text-gray-100 mb-4">Training Workers</h3>
              ...
              {['ckpt-worker-0', 'ckpt-worker-1'].map((container, idx) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={`w-3 h-3 rounded-full ${STATE_DOT[worker?.status ?? 'DEAD'] ?? 'bg-gray-500'}`} />
                        <div>
                          <p className="font-medium text-gray-200">Worker {idx}</p>
                          <p className="text-xs text-gray-500 font-mono">{container}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        {worker && (
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-red-900/50 hover:bg-red-800 disabled:bg-gray-800 disabled:text-gray-600 text-red-300 text-sm font-medium rounded-md transition-colors"
                        >
          ...
          {/* Recovery Timeline */}
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            <h3 className="text-lg font-semibold text-gray-100 mb-4">Event Timeline</h3>
            {timeline.length === 0 ? (
              <p className="text-sm text-gray-500">Events will appear here as they happen...</p>
            ) : (
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {timeline.map((event, i) => (
                  <div
                    key={i}
                    className={`flex items-start gap-3 px-3 py-2 rounded-md border-l-2 ${EVENT_COLORS[event.type]}`}
                  >
          ...
          {committedCheckpoints.length > 0 && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <h3 className="text-lg font-semibold text-gray-100 mb-4">Checkpoint History</h3>
              <div className="space-y-2">
                {committedCheckpoints.slice(-10).reverse().map(cp => (
                  <div
                    key={cp.checkpoint_id}
                    className="flex items-center justify-between px-3 py-2 bg-gray-800/50 rounded-md"
                  >
                    <div className="flex items-center gap-3">
                      <span className="w-2 h-2 rounded-full bg-blue-400" />
                      <span className="text-sm text-gray-300">Step {cp.step}</span>
                    </div>
          ...
          <div className="max-w-2xl mx-auto space-y-4 text-gray-400 text-sm">
            ...
            <div className="grid grid-cols-3 gap-6 mt-8 text-left">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs">Click "Kill" to simulate a node failure. The control plane detects the missed heartbeats within 15 seconds.</p>
              </div>
              <div className="space-y-2">
