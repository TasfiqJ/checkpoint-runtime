...
function CheckpointRow({ ckpt }: { ckpt: CheckpointInfo }) {
  ...
  return (
    <>
      ...
      >
        <td className="table-cell text-txt-3">
          <span className="inline-block w-4 text-txt-3/50 mr-1">{expanded ? '\u25BE' : '\u25B8'}</span>
          <span className="font-mono text-txt-2 text-xs">
            {shortId(ckpt.checkpoint_id, 16)}
          </span>
        </td>
        <td className="table-cell text-txt-3 font-mono text-xs">{ckpt.run_id.slice(0, 12)}</td>
      ...
      {expanded && (
        <tr>
          <td colSpan={7} className="px-4 py-4 bg-surface-0">
            <div className="pl-5">
              <h4 className="text-2xs font-semibold text-txt-3 uppercase tracking-wider mb-2.5">
                Shard Details ({ckpt.shard_ids.length} shards)
              </h4>
              {ckpt.shard_ids.length === 0 ? (
                <p className="text-sm text-txt-3">No shard data available.</p>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                  {ckpt.shard_ids.map((shardId, idx) => (
                    <div
                      key={shardId}
                      className="bg-surface-2 border border-line rounded-lg px-3 py-2 flex items-center justify-between"
                    >
                      <div>
                        <p className="text-xs font-mono text-txt-2">{shortId(shardId, 20)}</p>
                        <p className="text-2xs text-txt-3">Shard {idx}</p>
                      </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-6">
        ...
        <MetricCard label="Committed" value={committedCount} valueColor="text-ok" />
